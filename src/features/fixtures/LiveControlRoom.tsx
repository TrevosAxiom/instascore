            ...
            <Stack spacing={2.5}>
              ...
              {room.data.health.failed || room.data.health.stale ? (
                <Alert severity="warning">
                  One or more attached streams are stale, interrupted, or reporting synchronization
                  errors. Verify the Veo uplink and YouTube Live Control Room before kickoff.
                </Alert>
              ) : (
              ...
              <Box>
                ...
                <Grid container spacing={1.25}>
                      ...
                      <Paper variant="outlined" sx={{ p: 1.5, height: '100%' }}>
                        ...
                        {item.stale ? (
                          <Alert severity="warning" sx={{ mt: 1 }}>
                            Updates are more than five minutes old.
                          </Alert>
                        ) : null}
                        {item.errorMessage ? (
                        ...
                        <Stack direction="row" gap={1} sx={{ mt: 1 }}>
                          <Button
                            ...
                            to={`/fixtures/${item.fixtureUuid}`}
                          >
                            Open match
                          </Button>
                          <Button
                            size="small"
                            href={`https://studio.youtube.com/video/${item.videoId}/livestreaming`}
                            target="_blank"
                            rel="noreferrer"
                          >
                            YouTube Studio
                  ...
                  {!room.data.health.items.length ? (
                    <Grid size={12}>
                      <Alert severity="info">No fixtures have a broadcast attached yet.</Alert>
                    </Grid>
                  ) : null}
                </Grid>
              </Box>
              ...
              <Box>
                <Typography variant="h6" fontWeight={950}>
                  Fixture matching review
                </Typography>
                <Typography variant="body2" color="text.secondary" sx={{ mb: 1.5 }}>
                  High-confidence matches require both team names and a close scheduled kickoff.
                  Medium and low matches always require your approval.
                </Typography>
                <Stack spacing={1.25}>
                        ...
                        <Box sx={{ flexGrow: 1, minWidth: 0 }}>
                          <Stack direction="row" gap={1} alignItems="center" flexWrap="wrap">
                            ...
                            <Chip
                              ...
                              label={broadcast.status.replaceAll('_', ' ')}
                            />
                            {broadcast.concurrentViewers !== null ? (
                              <Chip
                                size="small"
                                label={`${broadcast.concurrentViewers} watching`}
                              />
                            ) : null}
                          </Stack>
                          <Typography variant="caption" color="text.secondary">
                            {broadcast.scheduledStart
                              ? new Date(broadcast.scheduledStart).toLocaleString()
                              : 'No scheduled start'}
                          </Typography>
                          <Stack spacing={0.75} sx={{ mt: 1 }}>
                                ...
                                >
                                  <Box sx={{ flexGrow: 1 }}>
                                    ...
                                    <Typography variant="caption" color="text.secondary">
                                      ...
                                      {new Date(
                                        `${suggestion.kickoffAt.replace(' ', 'T')}Z`,
                                      ).toLocaleString()}{' '}
                                  ...
                                  <Chip
                                    ...
                                    label={`${suggestion.score}% ${suggestion.confidence}`}
                                  />
                                  <Button
                                    size="small"
                                    variant="contained"
                                    onClick={() =>
                                      attach.mutate({
                                        fixtureUuid: suggestion.fixtureUuid,
                            ...
                            {!suggestions.length ? (
                              <Alert severity="info">
                                No likely fixture found. Create or reschedule the fixture, then
                                refresh.
                              </Alert>
                            ) : null}
                          </Stack>
                  ...
                  {!room.data.reviewQueue.length ? (
                    <Alert severity="success">
                      Every active channel broadcast is attached or complete.
                    </Alert>
                  ) : null}
                </Stack>
              </Box>
