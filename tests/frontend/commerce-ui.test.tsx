...
const ticket = {
  ...
  name: 'CFFL Finals Ticket',
  slug: 'cffl-finals',
  description: 'Entry to the championship game.',
  imageUrl: '',
  priceMinor: 500000,
  currency: 'NGN',
  stockQuantity: 40,
...
describe('commercial milestone UI', () => {
  it('shows catalogue products and creates a server-priced order', async () => {
    ...
    renderApp(<StorefrontPage />, {
      auth: adminAuth,
      api: { ...testApi, getCommerceCatalogue: () => Promise.resolve([ticket]), checkoutCommerce },
    });
    expect(await screen.findByText('CFFL Finals Ticket')).toBeInTheDocument();
    fireEvent.click(screen.getByRole('button', { name: 'Create order' }));
    await waitFor(() =>
      expect(checkoutCommerce).toHaveBeenCalledWith({
        items: [{ productUuid: 'product-1', quantity: 1 }],
        paymentMethod: 'manual',
        customerName: 'League Admin',
        deliveryAddress: '',
      }),
    );
  });
