...
final class FantasyScoringController {
	public function register(): void {
		...
		register_rest_route( 'instascore/v1', '/fantasy/games/(?P<uuid>[0-9a-f-]{36})/leagues', array(
			'methods'             => 'GET',
			'callback'            => fn( WP_REST_Request $request ): WP_REST_Response => $this->execute( fn(): array => FantasyScoringService::create()->leagues( get_current_user_id(), (string) $request['uuid'] ) ),
			'permission_callback' => array( $this, 'authenticated' ),
		) );
