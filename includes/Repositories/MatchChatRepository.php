...
final class MatchChatRepository {
	...
	public function messages( int $fixture_id, int $viewer_id, int $limit = 75 ): array {
		$rows = $this->database->get_results( $this->database->prepare( "SELECT m.*,p.uuid parent_uuid,p.display_name parent_display_name FROM {$this->prefix}chat_messages m LEFT JOIN {$this->prefix}chat_messages p ON p.id=m.parent_id WHERE m.fixture_id=%d AND m.status='active' ORDER BY m.id DESC LIMIT %d", $fixture_id, $limit ), ARRAY_A );
	...
	public function report( int $message_id, int $user_id, string $reason ): void { $this->database->replace( $this->prefix . 'chat_reports', array( 'uuid' => wp_generate_uuid4(), 'message_id' => $message_id, 'reporter_user_id' => $user_id, 'reason' => $reason, 'status' => 'open', 'created_at' => gmdate( 'Y-m-d H:i:s' ) ) ); $this->database->query( $this->database->prepare( "UPDATE {$this->prefix}chat_messages SET report_count=report_count+1 WHERE id=%d", $message_id ) ); }
	public function moderate( int $message_id ): void { $this->database->update( $this->prefix . 'chat_messages', array( 'status' => 'moderated', 'updated_at' => gmdate( 'Y-m-d H:i:s' ) ), array( 'id' => $message_id ) ); }
