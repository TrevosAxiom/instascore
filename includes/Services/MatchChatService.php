...
final class MatchChatService {
	...
	public function fantasy_post( string $game_uuid, array $input, int $user_id ): array {
		...
		return $this->present( $this->repository->create( array( 'fixture_id' => 0, 'room_type' => 'fantasy', 'room_id' => $game_id, 'user_id' => $user_id, 'user_uuid' => UserIdentity::uuid( $user_id ), 'display_name' => sanitize_text_field( (string) $user->display_name ), 'parent_id' => $parent_id, 'body' => $body ) ) );
	}
	...
	public function report( string $message_uuid, string $reason, int $user_id ): array { $message = $this->message( $message_uuid ); if ( (int) $message['user_id'] === $user_id ) { throw new ValidationException( array( 'message' => 'cannot_report_own_message' ) ); } $reports = $this->repository->report( (int) $message['id'], $user_id, sanitize_text_field( $reason ?: 'Inappropriate content' ) ); if ( $reports >= 3 ) { $this->repository->hold_for_review( (int) $message['id'] ); } return array( 'reported' => true, 'heldForReview' => $reports >= 3 ); }
